...
public class Test {
    ...
    public static void main(String[] args) throws IOException {
        // 写入文件的路径
        String filePath = "/home/xiuyuan.huang/test-es-mysql/trade/trade_test.txt";
        // 切分文件的路径
        String sqlitFilePath = "/home/xiuyuan.huang/test-es-mysql/trade";
...
//        System.out.println("每个子文件排序完毕！");
    ...
    public static void WriteData(String path,int CountNumbers) throws IOException {
        ...
        try {
            ...
            ps = conn.prepareStatement("select * from dw_trade_tmp limit 10000",
                    ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
            ps.setFetchSize(Integer.MIN_VALUE);
            rs = ps.executeQuery();
            List<String> listColumn = Arrays.asList("express_agency_fee","timeout_action_time","step_trade_status","step_paid_fee","part");
                ...
                for(int i = 1; i <= colCount; i++  )   {
                    ...
                    if(!listColumn.contains(name)) {
                        ...
                        if(value != null && !"".equals(value.trim())) {
                            ...
                            buffer.append("\"" + name + "\":\"" + value.replaceAll("\\s*","") + "\"");
                            if (i < colCount) {
                                buffer.append(",");
                            }
                        }
    ...
    public static void fileDataToEs(String filepath){
        //批量插入数据到es
        Client client = null;
        try {
            ...
            client = tClient.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("10.153.204.250"), 9500));
            ...
            while ((json = bufferedReader.readLine()) != null) {
                ...
                String routing = "";
                try {
                    int index1 = json.indexOf("tid");
                    int index2 = json.indexOf(",", index1);
                    index1 += 6;
                    id = json.substring(index1, index2 - 1);
                    ...
                    bulkRequest.add(client.prepareIndex("odps_trade_test", "dw_trade_tmp").setId(id).setRouting(routing).setSource(json));
                    //每一千条提交一次
                    if (count % 10000 == 0) {
